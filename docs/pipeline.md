...
> ```
>
> この条件は `preflight` ジョブが先に検出し、`correctness` と `security` を
> **skip** します（CI は赤くなりません）。Actions のログに警告が出て、
> `review:passed` は付きません。レビューが行われていないのに
> 「通った」と見えることはない、という状態です。
>
...
**確認すべき失敗:** レビュアーが**実行されたのに**構造化出力を返さなかった場合、
ジョブは `::error::...構造化出力を返しませんでした` で**失敗します**。これは意図した
挙動です（判定不能を「指摘なし」として素通りさせないため）。

上の「実行されなかった」ケース（skip）と区別してください。前者は赤、後者は灰色です。
いずれの場合も `review:passed` は付きません。`review:passed` は
`max_severity` が `none` / `low` / `medium` のいずれかだったときにだけ付きます。
