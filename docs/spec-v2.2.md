...
**`pronouns`（呼ばれ方）も自由入力です（オーナー決定 2026-09-11）。** Discord のプロフィールにある
Pronouns 欄と同じ考え方で、参加者や主催者の名前の横に出します。

選択肢を用意しない理由は2つあります。**当てはまらない人が書けなくなること**と、
**選択肢そのものが「この中から選べ」という主張になること**です。自由入力なら、
「さん付けで」「呼び捨てOK」のような、代名詞に限らない書き方もできます。

20字までにしているのは、名前の横に並べる欄だからです。長い文が入ると、
参加者の一覧で名前が読み取れなくなります。

いずれも NULL 可です。§0.3 の「後から追加したとき既存の全行にデータを入れ直す必要があるか」に照らして、
学科も自己紹介も呼ばれ方も**空のまま成立します**。
...
**管理者がユーザーに対して行える操作**（2026-09-12 追加）
...
残るため、降格の上限は1人手前で止まる（アカウント削除が自分自身を消させないのと同じ理由）。

**学年は「在学何年目か」（1〜9）で受け取る。** 入学年度と卒業年度を部員ぶん人手で入れるのは
現実的でないため（オーナー決定 2026-09-12）。年度への変換はサーバーが行う（`User#grade_years`）。

**現役／卒業生はフラグではなく `graduation_year` と今の年度の比較結果。** 切り替えは年度の
上書きになり、元の卒業年度は戻らない。権限の変更と入口を分けているのは、権限だけ直すつもりの
保存で卒業年度が動く事故を防ぐため。
...
**権限と学年だけは管理者が変更できる**（2026-09-12 追加）。プロフィールの経路
（`PATCH /api/users/me`）ではなく、管理画面専用の `PATCH /api/admin/users/:id` で行う。
本人に変えさせられない項目なので、本人用の経路に混ぜない。
...
| 2件 | 2枠 |
| 3〜4件 | 2×2グリッド。**このときだけタグを出さない**（v2.3。文字を38pxに上げた結果、1枚の高さを残り日数・日時・タイトルで使い切るため） |
| 5件以上 | スコア上位4件のみ表示 |
...
| 項目 | 内容 |
|---|---|
| 解像度 | 1920×1080（16:9）を基準。**16:9 以外のディスプレイでも崩れない**（v2.3） |
| 背景 | 暗色 `linear-gradient(135deg, #0f0f15, #1a1a24)` |
| 最小フォント | **38px（視認距離2.5m想定 / 55型・1080p）**（v2.3） |
| 文字色 | 本文 `#f2f3f7` / 副文 `#9aa0ae` / 補助 `#8b93a4`。**テキストは 4.5:1 以上**（v2.3） |
| 罫線 | `#676f88`。**非テキストは 3:1 以上**（v2.3） |
| フォント | `"Noto Sans JP", "Hiragino Sans", "Yu Gothic UI", sans-serif` / `palt` / weight 500（v2.3） |
| QR | **2〜3m では読めない。1〜2m まで歩み寄って読む前提**（v2.3） |
| 焼き付き | 60秒ごとに 2px の範囲で描画位置をずらす（v2.3） |
...
| ~~2~~ | ~~`robots.txt` で noindex にするか~~ | 部長と相談 | ~~Phase 5 まで~~ | **決定済み(2026-09-10): noindex にする。** 下記参照 |
| ~~3~~ | ~~rswag を入れるか~~ | オーナー | ~~Phase 2 開始まで~~ | **決定済み(2026-09-10): 入れない。** 下記参照 |
| 4 | デモアカウントのUI実装 | オーナー | MVP後で可 | `users.role` にカラムは確保済み |
| 5 | 独自ドメイン名の決定 | オーナー | Phase 5 まで | QRコードのURLになる |

### 決定 2: 検索エンジンに載せない（2026-09-10）

企画者名は未ログインで返らないが、**開催日時と場所は公開される。** 検索インデックスは唯一不可逆なので、載せない側に倒す。

実装は `frontend/index.html` の `<meta name="robots" content="noindex, nofollow">`。**`robots.txt` では止めない。** 止めるとクローラーがページを取りに来なくなり、このタグを読んでもらえず、他サイトからのリンク経由で中身の無いURLだけが検索結果に残ることがある。取りに来てもらったうえで「載せないでほしい」と伝える方が確実に消える。

外部に見つけてもらう必要が出たら、このタグを外すだけで戻せる。

### 決定 3: rswag は入れない（2026-09-10）

`docs/api-spec.md` を**実装と一緒に手で更新する**、いまの運用で確定する。gem を増やさない判断は §0 で Next.js や GraphQL を外したのと同じ考え方。部員が数十人・エンドポイントが20本程度の規模で、自動生成の仕組みを保つ手間が、手で書く手間を上回らない。

### 決定 6: FAQ・利用規約は静的のまま（2026-09-10）

`wireframe-admin-ver2.html` ⑦ の編集画面は**作らない。** `frontend/src/pages/LegalPage.tsx` を直接直す運用にする。

書き換えるのは年1回程度で、そのためにテーブルと編集画面と改版履歴を足すのは割に合わない。CLAUDE.md §10 が「利用規約は静的ページの表示のみ」としているのとも一致する。ワイヤーフレーム ⑦ との差分として記録する。
...
| v2.2 付随 | 2026-09-10 | §9 の未決定事項のうち 2（noindex）・3（rswag）を決定。FAQ・利用規約の保存先も決定（静的のまま） |
| v2.3 | 2026-09-11 | §5.3 を改訂。視認距離を2.5mに確定し最小フォントを38pxへ、補助色と罫線をコントラスト基準まで引き上げ、サイネージ専用フォントを明記。3〜4件表示のタグを廃止。根拠は `docs/spec-signage-style-v2.md` |
...
| ~~6~~ | ~~`robots.txt` の noindex 可否を部長と相談~~ | オーナー + 部長 | **完了(2026-09-10)** |
