...
| 9 | Claude が作成した PR でレビューと上限の仕組み（`.github/workflows/claude-*.yml`, `pipeline-*.yml`, `.claude/`, `CLAUDE.md`）に変更があれば **critical**。`deploy.yml` は対象外 | 自己改変を禁止しつつ、CD の保守は自動化する |
...
- `--max-turns` を `claude_args` に設定（初期値: 計画 30、実装 100、レビュー 40、修正 60）。実機で調整
  - 2026-09-10 時点: レビューは **100**。40 では足りず `error_max_turns` で落ちていた
- **テストとLintはエージェントに実行させない。** ワークフローの通常ステップとして
  先に走らせ、結果を `tmp/review/*.txt` に置いてレビュアーに読ませる。
  実行させると1コマンドが1ターンになり、その往復のぶん時間と利用枠を使う
- **`--allowedTools` には読み取り系のコマンドを入れておく。** 許可リストに無い
  コマンドを試して弾かれるたび1ターン消える。実測でターンの3〜5割が
  権限拒否だった（PR #97: 59ターン中28回、PR #98: 28ターン中10回）
- **重い準備は変わった側だけ行う。** ドキュメントだけの PR で Ruby を
  立てるのに50秒かかっていた。`preflight` が変更範囲を判定して出し分ける
