...
| 新WFの要素 | 現行 | 扱い |
|---|---|---|
| ② 停止中バッジ・停止／停止解除 | `spec-v2.2.md` §0.3 で `suspended_at` は「作らない」 | T7-4 で扱う。マイグレーションが要るので `spec-v2.2.md` §2 の変更 |
| ②③ 学科 | `users` に列が無い | 同上 |
...
| T7-4 | ② ユーザー一覧（`/admin/users`）＋ 停止・学科 | 1日 | **`spec-v2.2.md` §2 にカラムを追加してから着手する。オーナーの承認が要る** |
