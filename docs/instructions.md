...
| 件 | 内容 | 選択肢 |
|---|---|---|
| 開催済みイベントの扱い | `GET /api/events` は `status: recruiting` で絞るだけで、開催日が過去のイベントを外していない。トップと一覧に「開催済み」の企画が並ぶ。過去を外しているのはサイネージ用の `Event.spotlight_targets` だけ | ①API側の既定に「開催日が今日以降」を足す（`api-spec.md` §2 の改訂が要る） ②表示側で「開催済み」と出す（現状。判定規則がRubyとTSの2箇所に増える） ③owner が `completed` に変える運用 |
