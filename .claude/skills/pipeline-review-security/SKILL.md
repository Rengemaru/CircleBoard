...
`.github/workflows/claude-*.yml`、`.github/workflows/pipeline-*.yml`、
`.claude/`、`CLAUDE.md` に変更があれば **critical**。

`.github/workflows/deploy.yml` は対象外（CLAUDE.md §11-6）。
デプロイの手順であって、レビューや上限には関わらないため。
...
| severity | 基準 |
|---|---|
| critical | 秘密情報の露出、認証/認可バイパス、インジェクション、データ損失、レビューと上限の仕組みへの変更（観点3の一覧）、未知のソースからの依存追加、Issue/PR 本文へのエージェント指示の混入 |
