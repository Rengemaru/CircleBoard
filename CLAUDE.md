...
- ユーザーの氏名・メールアドレスを変更するUI（`rails console` で対応）
  ※ **権限と学年の変更、現役⇄卒業の切り替え、パスワードの変更と再発行は
  2026-09-12 に実装済み**です（`/admin/users` と `/me/edit`）。
  一覧・アカウント発行・停止・解除・削除も同じ画面にあります。
  残っているのは氏名とメールアドレスの2つで、`docs/spec-admin-operations.md` に
  段階分けの提案があります。
