...
- プロフィールの画像アップロードと公開設定（項目ごとの公開/非公開）
  ※ プロフィールの**表示・編集は 2026-09-10 に実装済み**です（`docs/spec-my-page.md`）
- co_organizer の UI と API（`owner_id` のみで運用）
  ※ owner の付け替えも作りません。**owner 自身はプロジェクトを脱退できません**
- 企画ごとのFAQ
- 利用規約の同意フロー（静的ページの表示のみ）
- パスワードのリセットを**メールで送る**仕組み（通知機能を作らないため。下記参照）
- ユーザーの**メールアドレス**を変更するUI（`rails console` で対応）
  ※ **メールアドレス以外はすべて画面から編集できます**（オーナー決定 2026-09-12）。
  権限・学年・現役⇄卒業・パスワード・氏名は `/admin/users` と `/me/edit` にあります。
  一覧・アカウント発行・停止・解除・削除も同じ画面です。
  パスワードは**管理者が再発行して口頭で伝える**形です。リセットリンクは送りません
- 統計ダッシュボード
- 通知機能（メール / LINE / Discord）
- スマホ専用ナビ（ボトムナビ・FAB）※レスポンシブ対応はする
  ※ **企画詳細の操作バーは例外**（Issue #302、オーナー決定 2026-09-13）。
  ナビゲーションではなく、その画面の主操作を画面下端に固定するものです。
  本文が長いと操作が画面外に流れて押せなくなるため。
  smarthr-ui の `FloatArea` を使います
- 「過去の企画」セクション
...
**これは目安ではなく上限です。パイプライン経由かどうかに関わらず、
このリポジトリに出すすべての PR に適用します。**

- 数えるのは**追加行と削除行の合計**
- テストコードは**別枠で数え、上限はありません**（300行の圧力でテストが削られるのを防ぐため）
- ロックファイル・生成物・バイナリは集計から除外されます
- `pipeline-line-gate.yml` が自動で数えて落とします

**1つの Issue が300行に収まらないときは、Issue の中で作業を分割し、
PR を複数に分けて出します。** 「1つの Issue = 1つの PR」ではありません。
先にマージできる単位から順に出し、1本ずつレビューを受けます。

分割の単位は「マージした時点で main が壊れていないこと」です。
UI とその API を同時に足す必要があるなら、それは1本にまとめます。

### 11-2. 着手前に main からブランチを切る

- **既存のブランチに相乗りしない。** 無関係なコミットが PR に混ざり、
  レビューする人がどれが今回の変更か分からなくなります
- main に直接コミットしない
- ブランチ名は `feat/event-crud`、`fix/signage-token-404`（§5）

### 11-3. コミットは小さい単位で刻む

1つのコミットに1つの意味のある変更だけを入れます。作業の最後にまとめて
1コミットにしません。レビューする人が差分を1つずつ追えることを優先します（§7）。

コミットメッセージは Conventional Commits に従い、**なぜそうしたか**を本文に書きます。
